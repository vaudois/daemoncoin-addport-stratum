...
if [[ ("$stratumyesnot" == "n" || "$stratumyesnot" == "N" || "$stratumyesnot" == "not" || "$stratumyesnot" == "NOT" || "$stratumyesnot" == "no" || "$stratumyesnot" == "NO") ]]; then
	...
	echo -e "$CYAN --------------------------------------------------------------------------------------- 	$COL_RESET"
	echo
	read -r -e -p " Please enter your stratum desired to use, example stratum :" stratumselected
	echo
	echo -e "$CYAN --------------------------------------------------------------------------------------- 	$COL_RESET"
	echo
	...
	if [[ -z "${stratumsplit[1]}" ]]; then
		...
		if [[ ! -f "${filerun}" ]]; then
        	...
        	sudo chgrp ${USERSERVER} ${PATH_STRATUM}/${filerun}
     		sudo chown ${USERSERVER} ${PATH_STRATUM}/${filerun}
			sleep 1
			echo '#!/usr/bin/env bash
			ulimit -n 10240
			ulimit -u 10240
        	...
        	sudo chgrp ${USERSERVER} ${PATH_STRATUM}/config/${filerun}
     		sudo chown ${USERSERVER} ${PATH_STRATUM}/config/${filerun}
		fi
	fi
	...
	sudo chgrp ${USERSERVER} ${PATH_STRATUM}/config/stratum.${coinsymbollower}
	sudo chown ${USERSERVER} ${PATH_STRATUM}/config/stratum.${coinsymbollower}
	sleep 1
else
	# New coin stratum start file
	echo '#####################################################
...
if [[("$CREATECOIN" == 'true')]]; then
	...
	sudo chgrp ${USERSERVER} ${absolutepath}/${installtoserver}/daemon_builder/.addport.cnf
	sudo chown ${USERSERVER} ${absolutepath}/${installtoserver}/daemon_builder/.addport.cnf
