...
if (( $EUID == 0 )); then
	...
	if [[ ("${EXIT}" == "false") ]]; then
		...
		hide_output sudo apt -y install build-essential autotools-dev automake pkg-config qtwayland5 python3 ccache libssl-dev libevent-dev libboost-all-dev libminiupnpc-dev \
		libqt5gui5 libqt5core5a libqt5webkit5-dev libqt5dbus5 qttools5-dev qttools5-dev-tools libprotobuf-dev protobuf-compiler libqrencode-dev libzmq3-dev libgmp-dev \
		cmake libunbound-dev libsodium-dev libunwind8-dev liblzma-dev libreadline6-dev libldns-dev libexpat1-dev libpgm-dev libhidapi-dev libusb-1.0-0-dev \
		libudev-dev libboost-chrono-dev libboost-date-time-dev libboost-filesystem-dev libboost-locale-dev libboost-program-options-dev libboost-regex-dev \
		libboost-serialization-dev libboost-system-dev libboost-thread-dev doxygen graphviz default-libmysqlclient-dev libnghttp2-devlibrtmp-dev libssh2-1 \
		libssh2-1-dev libldap2-dev libidn11-dev libpsl-dev libnatpmp-dev systemtap-sdt-dev libmagic-dev libtool libbrotli-dev liblbfgs-dev liblbfgsb-dev libssh-dev
