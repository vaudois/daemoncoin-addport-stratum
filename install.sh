...
clear
	...
	sudo sed -i 's#bchdons#'$BCHDEP'#' conf/functions.sh#!/bin/bash#!/bin/bash
################################################################################
#
# Current Created by : Vaudois
# web: https://coinXpool.com
...
if (( $EUID == 0 )); then
	...
	if [[ -f "${absolutepath}/${installtoserver}/conf/info.sh" ]]; then
		...
		if [[ ("$VERSION" == "$TAG") ]]; then
			...
			\n\nIMPORTANT: It is not necessary to install this version because you already have it, thanks"
			cd ..
			sudo rm -rf ${absolutepath}/${installdirname}/
			cd ~
			clear
			exit;
		else
	...
	if [[ ("${INSTVERSION}" == "${TAG}") ]]; then
		...
		SCSCRYPT=/etc/screen-scrypt.sh
		if [[ ! -f "$SCSCRYPT" ]]; then
			hide_output sudo cp -r ${absolutepath}/${installdirname}/utils/screen-scrypt.sh /etc/
			hide_output sudo chmod +x /etc/screen-scrypt.sh
		fi

		EDITCONFAPP=/usr/bin/editconf.py
		if [[ ! -f "$EDITCONFAPP" ]]; then
			hide_output sudo cp -r ${absolutepath}/${installdirname}/conf/editconf.py /usr/bin/
			hide_output sudo chmod +x /usr/bin/editconf.py
		fi
	...
	if [[ "${FILEINFO}" == "true" ]]; then
		...
		if [ -z "${path_stratum}" ]; then
			...
			echo
			echo
			echo -e "$RED You are Cancelled this install! to run again => $COL_RESET$YELLOWsudo bash ${absolutepath}/${installdirname}/install.sh $COL_RESET"
			echo
			exit
		fi 
	...
	if [[ "${NEWVERSION}" == "$TAG" ]] || [[ "${INSTVERSION}" == "$TAG" ]]; then
		...
		hide_output sudo cp -r ${absolutepath}/${installdirname}/utils/addport.sh /usr/bin/addport
		hide_output sudo chmod +x /usr/bin/addport
