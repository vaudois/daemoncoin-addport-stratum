...
if (( $EUID == 0 )); then
	...
	if [[ ("${EXIT}" == "false") ]]; then
		...
		hide_output sudo apt -y install build-essential autotools-dev automake pkg-config qtwayland5 libssl-dev libevent-dev libboost-all-dev libminiupnpc-dev \
		...
		libgmp-dev cmake libunbound-dev libsodium-dev libunwind8-dev liblzma-dev libreadline6-dev libldns-dev libexpat1-dev libpgm-dev libhidapi-dev libusb-1.0-0-dev \
		...
		libboost-serialization-dev libboost-system-dev libboost-thread-dev python3 ccache doxygen graphviz default-libmysqlclient-dev libnghttp2-devlibrtmp-dev \
		libssh2-1 libssh2-1-dev libldap2-dev libidn11-dev libpsl-dev libnatpmp-dev systemtap-sdt-dev libmagic-dev libtool libbrotli-dev liblbfgs-dev liblbfgsb-dev
		if [[ ("${DISTRO}" == "18") ]]; then
